...
Star8::Star8(int X, int Y, Shape *parent)
    : Shape(parent)
{
    centreX=X;
    centreY=Y;
...
void Star8::Centre(double &x, double &y)
{
    x=centreX;
    y=centreY;
}

void Star8::doPainting() {
  ...
  QPolygon polygon({QPoint(coordinates[0], coordinates[1]),QPoint(coordinates[2], coordinates[3]),
                    ...
                    QPoint(coordinates[24], coordinates[25]),QPoint(coordinates[26], coordinates[27]),
                    QPoint(coordinates[28], coordinates[29]),QPoint(coordinates[30], coordinates[31])
                    });
